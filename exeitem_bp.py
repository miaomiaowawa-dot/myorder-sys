...
def get_all_items():
    try:
        ...
        #group by exetime and calcute the total price
        grouped_items = {}

        for item in items:
            date_str = item['exetime'].strftime('%Y-%m-%d') if isinstance(item['exetime'], datetime) else item['exetime']
            ...
            grouped_items[date_str]['items'].append({
                ...
                'item_price': float(item['item_price']),  # 确保是数值类型
                'item_remark': item['item_remark']
            })

            # 累加总价
            grouped_items[date_str]['total_price'] += float(item['item_price'])
    ...
    try:
        ...
        for order in started_orders:
            ...
            result.append({
                ...
                'recent_items': order_items[:12]  # 显示最近5个已使用项目
            })
...
def add_exeitem():
    ...
    try:
        ...
        # 7. 更新订单状态
        update_order_status(data['record_id'])
...
def update_order_status(order_id):
    ...
    try:
        ...
        used_services = result['used_services'] or 0
        pending_services = result['pending_services'] or 0
        started_services = result['started_services'] or 0
    ...
    try:
        ...
        query = """
            SELECT 
                ...
                (os.quantity - os.completed_quantity) as remaining_quantity,
        ...
        for item in results:
            ...
            orders[order_id]['services'].append({
                ...
                'used_count': used_count,
                'item_records': item_records,  # 包含所有item记录
                'progress': round((item['completed_quantity'] / total_quantity) * 100, 1) if total_quantity > 0 else 0
            })
